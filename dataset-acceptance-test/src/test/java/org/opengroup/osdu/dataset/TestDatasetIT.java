...
public final class TestDatasetIT extends TestBase {
    ...
    private CloseableHttpResponse sendDeleteDatasetRequest(String metadataRecordId) throws Exception {
        ...
        CloseableHttpResponse deleteResponse = TestUtils.send(String.format("metadataRecord/%s/softDelete", URLEncoder.encode(metadataRecordId, StandardCharsets.UTF_8)), "POST",
    ...
    public static String getAclSuffix() {
        String environment = StringUtils.isEmpty(TestUtils.getEnvironment()) ? StringUtils.EMPTY : TestUtils.getEnvironment();
        if (StringUtils.isNotEmpty(environment) && "empty".equalsIgnoreCase(environment)) {
            environment = "";
        }
    ...
    public void validateRetrievalInstructions(RetrievalInstructionsResponse retrievalInstructions,
        ...
        retrievalInstructions.getDatasets().stream().forEach(
                datasetRetrievalProperties ->
                        assertEquals(TokenTestUtils.getProviderKey(), datasetRetrievalProperties.getProviderKey()));
    }

    public void validateStorageInstructions(StorageInstructionsResponse storageInstructions) {
        assertEquals(TokenTestUtils.getProviderKey(), storageInstructions.getProviderKey());
